...
test('planning step shows the waiting state', async () => {
...
test('blocked step shows the failed state', async () => {
  useStore.getState().selectStep('sy2');
  render(<ReviewSummary />);
  await waitFor(() => expect(screen.getByText('막힘')).toBeInTheDocument());
});
